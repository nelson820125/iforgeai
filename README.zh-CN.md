...
面向软件交付团队的多平台多角色 AI Agent 工具包，支持 **GitHub Copilot**、**Claude Code**、**Codex CLI** 与 **TraeCN**。
...
cd iforgeai
pwsh ./install.ps1    # 选择平台：1 Copilot（或 3 两者都装）
```

在 VS Code 中：
1. `Ctrl+Shift+P` → **Developer: Reload Window**
2. 打开项目工作区
3. Copilot Chat → 运行 `/init-project` 搞建 `.ai/` 目录结构
4. 切换到 Agent 模式 → 选择 `digital-team` → 描述本次迭代目标

![iforgeAI on VS Code - Agent 选择器](public/assets/VSCode-1.png)

![iforgeAI on VS Code - 对话界面](public/assets/VSCode-2.png)
...
git clone https://github.com/nelson820125/iforgeai.git
cd iforgeai
pwsh ./install.ps1          # 选择平台：2 Trae（或 3 两者都装）
```

安装后，所有角色技能将写入全局目录 `~/.trae-cn/skills/`（Trae 国际版为 `~/.trae/skills/`），编码规范 instructions 写入对应的 `instructions/` 目录。

安装完成后，在 Trae 对话框中使用 `@角色名` 唤醒对应角色：

```
@digital-team      — 协调者；使用 /init-project 初始化新项目
@product-manager   — 需求分析
@architect         — 架构设计
@dba               — 数据库设计
```

**方式二 — 一键添加共享智能体** *（仅限 Trae CN）*

点击链接即可将智能体添加到 Trae 工作区（无需安装）：

| 角色 | 链接 |
|---|---|
| digital-team（协调者） | [添加智能体](https://s.trae.com.cn/a/20b610?region=cn) |
| product-manager（产品经理） | [添加智能体](https://s.trae.com.cn/a/0774f5?region=cn) |
| project-manager（项目经理） | [添加智能体](https://s.trae.com.cn/a/a47648?region=cn) |
| architect（架构师） | [添加智能体](https://s.trae.com.cn/a/2215d0?region=cn) |
| dba（数据库设计师） | [添加智能体](https://s.trae.com.cn/a/15f783?region=cn) |
| ui-designer（UI 设计师） | [添加智能体](https://s.trae.com.cn/a/9d7e92?region=cn) |
| frontend-engineer（前端工程师） | [添加智能体](https://s.trae.com.cn/a/211c10?region=cn) |
| dotnet-engineer（.NET 工程师） | [添加智能体](https://s.trae.com.cn/a/637798?region=cn) |
| java-engineer（Java 工程师） | [添加智能体](https://s.trae.com.cn/a/37cb8a?region=cn) |
| python-engineer（Python 工程师） | [添加智能体](https://s.trae.com.cn/a/8c1a62?region=cn) |
| qa-engineer（测试工程师） | [添加智能体](https://s.trae.com.cn/a/122a53?region=cn) |
| plan（技术方案） | [添加智能体](https://s.trae.com.cn/a/1e717a?region=cn) |
| devops-engineer（DevOps 工程师） | [添加智能体](https://s.trae.com.cn/a/121c87?region=cn) |

**初始化新项目**

在 Trae 对话框中，使用 `@digital-team` 并执行 `/init-project` 来初始化项目的 `.ai/` 目录结构：

```
@digital-team /init-project
```

引导式问答将创建 `.ai/context/workflow-config.md`，并将相关编码规范从全局 instructions 目录复制到 `.trae/rules/`。

![iforgeAI on Trae - 智能体列表](public/assets/Trae-1.png)

![iforgeAI on Trae - 对话界面](public/assets/Trae-2.png)

![iforgeAI on Trae - 初始化项目](public/assets/Trae-3.png)
...
```sh
# 复制到项目根目录（或 ~/.claude/ 对所有项目生效）
cp zh-CN/claude-code/CLAUDE.md /path/to/your/project/CLAUDE.md
```

在 Claude Code 对话中以触发词开始：`PM：` / `架构师：` / `前端工程师：` …
