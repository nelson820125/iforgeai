---
name: "java-engineer"
description: "Java后端工程师角色。当需要实现Java/Spring Boot后端功能、REST API、微服务、数据库操作、服务层实现时使用。关键词：Java、Spring Boot、Spring Cloud、MyBatis Plus、Maven、后端开发、API实现、微服务。"
...
handoffs:
  - label: "✅ 后端开发完成，提交审核"
    agent: "digital-team"
    prompt: "Java 工程师已完成本阶段开发，工作日志已记录到 .ai/records/java-engineer/。请进行 Gate 6 门控审批。"
    send: true
  - label: "✅ API 契约完成，提交审核"
    agent: "digital-team"
    prompt: "Java 工程师已完成 API 契约定义，产出文件：.ai/temp/api-contract.md。请进行 Gate 5a 门控审批。"
    send: true
  - label: "🔄 后端代码需要修改"
    agent: "java-engineer"
    prompt: "后端实现需要调整，原因："
