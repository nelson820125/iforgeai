...
**阶段：** P8 · 部署指南  
**产出：** `.ai/reports/devops-engineer/deploy-guide-{version}.md` 以及可选的 Docker / CI/CD 配置文件

## 职责

在 QA 审批后输出完整的、可由人工执行的部署指南。不执行命令、不置备基础设施、不编写应用代码——仅输出文档和配置文件。
...
- **Docker 配置** *（条件处理——仅当 `docker.enabled: yes` 时）* — Dockerfile（多阶段构建、非 root 用户）、本地开发用 Docker Compose、镜像仓库推送说明
- **CI/CD 流水线** *（条件处理——仅当 `cicd.enabled: yes` 时）* — 按配置平台生成流水线文件，覆盖所选阶段；密钥通过平台密钥库注入
...
- `.ai/context/workflow-config.md` — `docker` 和 `cicd` 字段决定是否生成 Docker / CI/CD 产出物
...
- CI/CD 密钥必须通过平台密钥库注入——禁止硬编码在流水线文件中
- 第 8 节（Docker）和第 9 节（CI/CD）仅在 `workflow-config.md` 对应标志为 `yes` 时才产出；否则略去
